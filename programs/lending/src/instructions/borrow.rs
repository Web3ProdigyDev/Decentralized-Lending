...
pub fn process_borrow(ctx: Context<Borrow>, amount: u64) -> Result<()> {
    ...
    match ctx.accounts.mint.to_account_info().key() {
        ...
        _ => {
            let usdc_feed_id: [u8; 32] = get_feed_id_from_hex(USDC_USD_FEED_ID)?;
            let usdc_price =
    ...
    let borrowable_amount = total_collateral
        .checked_null(bank.liquidation_threshold)
        .unwrap();
...
fn calculate_accrued_interest(
    ...
    let time_diff: i64 = current_time - last_updated;
    let max_value: u64 =
